package net.celsiusqc.cp_tweaks.entity.custom;

import net.minecraft.world.entity.LivingEntity;
import net.minecraft.world.entity.ai.goal.LeapAtTargetGoal;
import net.minecraft.world.entity.Mob;
import net.minecraft.world.phys.Vec3;

public class CustomLeapAtTargetGoal extends LeapAtTargetGoal {
    private final Mob mob;
    private final float yd;
    private LivingEntity target;

    public CustomLeapAtTargetGoal(Mob mob, float yd) {
        super(mob, yd);
        this.mob = mob;
        this.yd = yd;
    }

    @Override
    public boolean canUse() {
        if (this.mob.isVehicle()) {
            return false;
        } else {
            this.target = this.mob.getTarget();
            if (this.target == null) {
                return false;
            } else {
                double d0 = this.mob.distanceToSqr(this.target);
                // Increase the max range from which the mob will attempt to leap
                if (!(d0 < 9.0D) && !(d0 > 64.0D)) {
                    if (!this.mob.onGround()) {
                        return false;
                    } else {
                        return this.mob.getRandom().nextInt(reducedTickDelay(10)) == 0;
                    }
                } else {
                    return false;
                }
            }
        }
    }

    @Override
    public void start() {
        Vec3 vec3 = this.mob.getDeltaMovement();
        Vec3 vec31 = new Vec3(this.target.getX() - this.mob.getX(), 0.0D, this.target.getZ() - this.mob.getZ());
        if (vec31.lengthSqr() > 1.0E-7D) {
            // Increase the scale factor for a longer horizontal leap
            vec31 = vec31.normalize().scale(0.8D).add(vec3.scale(0.2D));
        }

        // You can also adjust the yd variable (used in super constructor) if you want to change the leap height.
        this.mob.setDeltaMovement(vec31.x, (double)this.yd, vec31.z);
    }

}
